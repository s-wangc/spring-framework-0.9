...
<P>
    <B>Owner:</B>
    <c:out value="${command.owner.firstName}"/>
    <c:out value="${command.owner.lastName}"/>
<P>
<FORM method="POST">
    <B>Name:</B>
        ...
        <SELECT name="typeId">
            <c:forEach var="typeEntry" items="${types}">
                <c:if test="${command.typeId == typeEntry.key}">
                    <OPTION selected="<c:out value="${command.typeId}"/>" value="<c:out value="${typeEntry.key}"/>">
                        <c:out value="${typeEntry.value.name}"/></OPTION>
                </c:if>
                <c:if test="${command.typeId != typeEntry.key}">
